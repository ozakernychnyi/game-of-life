...
func initPattern() [columns][rows]int {
...
func createUI(grid [columns][rows]int) string {
	...
	for i := 0; i < rows; i++ {
		s = s + createColumn(grid[i]) + horizontal
	}
...
// createColumn - creates vertical lines for grid. Also is responsible to put live cells
// into grid based on row slice, row slice consists live cells as elements with value 1.
// Ranging this raw tells us which places live cells hold in this grid row.
func createColumn(row [columns]int) string {
	c := []rune{'|'}
	for _, v := range row {
		if v == 1 {
			c = append(c, []rune{' ', '>', '<', ' ', '|'}...)
